...
void ecc_point_set_str(struct ecc_point *res, const struct string_st *str, const struct ecc_curve *curve) {
    if (res == NULL) return;
    ecc_point_clear(res);
    if (curve == NULL || str == NULL) return;
    unsigned tag = tlv_get_tag(str->data);
    if ((tag | 1) != (ECC_POINT_TLV | 1)) return;

    _integer_set_tlv(res->x, str);
...
void ecc_point_get_str(const struct ecc_point *P, struct string_st *res) {
    ...
    _integer_get_tlv(P->x, res, integer_get_ui(P->y) % 2 + ECC_POINT_TLV);
}
